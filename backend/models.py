...
class PhraseCheckInRow(Base):
    ...
    q1_phrase_recalled = Column(String(8), nullable=False)   # yes | no
    q2_recall_mode = Column(String(20), nullable=True)       # spontaneous | deliberate
    q3_timing = Column(String(10), nullable=True)            # before | during | after
    q4_effect = Column(String(20), nullable=True)            # helpful | neutral | distracting
    q5_situation_text = Column(Text, nullable=True)
    q6_attempted_recall = Column(String(8), nullable=True)   # yes | no
    q7_additional_text = Column(Text, nullable=True)
    timestamp = Column(DateTime(timezone=True), nullable=True)
    client_source = Column(String(20), nullable=False, server_default="web")
...
class PhraseCheckInCreate(BaseModel):
    ...
    @field_validator("client_source")
    @classmethod
    def validate_client_source(cls, v: str) -> str:
        if v not in {"web", "app"}:
            raise ValueError("client_source must be 'web' or 'app'")
        return v
